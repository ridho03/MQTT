...
<div class="max-w-7xl mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center space-x-4">
            <a href="{{ route('emission.monitoring') }}"
            ...
            <!-- <div class="text-gray-600">
                ...
                @if(Auth::user()->isAdmin())
                    <span class="inline-block bg-yellow-400 text-yellow-900 text-xs font-semibold px-2 py-1 rounded ml-2">
                        Administrator
                    </span>
                @endif
            </div> -->
            ...
            <!-- <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit"
                        class="px-3 py-2 rounded-lg text-sm bg-red-500 text-white hover:bg-red-600 flex items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm">
                ...
                <p class="text-2xl font-bold text-orange-600">
                    {{ number_format($emissionStats['today_emissions'], 2) }} kg
                </p>
            </div>
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="text-center">
                                    ...
                                    <p class="font-semibold text-orange-600">
                                        {{ number_format($vehicle['daily_emissions_kg'], 2) }} kg
                                    </p>
                                </div>

                                <div class="flex items-center space-x-2">
                                    <div class="w-3 h-3 rounded-full
                                        {{ $vehicle['sensor_status'] === 'active' ? 'bg-green-500' :
                                           ($vehicle['sensor_status'] === 'error' ? 'bg-red-500' : 'bg-gray-400') }}">
                                    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow border border-gray-100">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('transactions.index') }}" class="text-sm text-primary hover:underline">
                View All
            </a>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium uppercase">Total</th>
                    ...
                    @forelse($transactions->take(5) as $transaction)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="px-2 py-1 text-xs rounded-full {{ $colors[$transaction->status] ?? 'bg-gray-200 text-gray-700' }}">
                                    {{ $labels[$transaction->status] ?? ucfirst($transaction->status) }}
                                </span>
                            </td>
                        </tr>
                    @empty
