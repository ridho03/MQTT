...
<div class="max-w-7xl mx-auto p-6 bg-white rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-6">
        ...
        @if(!Auth::user()->isAdmin())
        <a href="{{ route('carbon-credits.create') }}"
           class="inline-flex items-center px-4 py-2 bg-primary text-white rounded hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-primary">
    ...
    @if($vehicles->count() > 0)
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 rounded-lg border border-gray-200"
                   role="table" aria-label="Daftar Kendaraan">
                <thead>
                    <tr>
                    ...
                    <tr class="hover:bg-gray-50 focus-within:bg-gray-100 transition-colors duration-200" tabindex="0">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm">
                            @switch($vehicle->status)
                                @case('pending')
                                    <span class="inline-flex items-center px-2 py-1 rounded-full bg-yellow-100 text-yellow-800 text-xs font-semibold">
                                ...
                                @case('rejected')
                                    ...
                                    @break
                                @case('sold')
                                    <span class="inline-flex items-center px-2 py-1 rounded-full bg-gray-200 text-gray-600 text-xs font-semibold">
                                        Terjual
                                    </span>
                                    @break
                            @endswitch
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm">
                            <div class="flex space-x-2">
                                ...
                                @if(!Auth::user()->isAdmin())
                                    {{-- EDIT --}}
                                    <a href="{{ route('carbon-credits.edit', $vehicle->id) }}"
                                       title="Edit"
                                       class="inline-flex items-center px-2 py-1 bg-yellow-400 text-white rounded hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-500">
                                        <i class="fas fa-edit" aria-hidden="true"></i>
                                    </a>

                                    {{-- HAPUS --}}
                                    <form method="POST"
                                          action="{{ route('carbon-credits.destroy', $vehicle->id) }}"
                                        ...
                                        <button type="submit"
                                                title="Hapus"
                                                class="inline-flex items-center px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-700">
                                            <i class="fas fa-trash" aria-hidden="true"></i>
                                        </button>
                                    </form>
                                @endif

                                {{-- ADMIN: Approve / Reject kendaraan --}}
                                @if(Auth::user()->isAdmin() && $vehicle->status === 'pending')
                                    <form method="POST"
                                          action="{{ route('carbon-credits.approve', $vehicle->id) }}"
                                          style="display: inline;">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                                title="Setujui"
                                                onclick="return confirm('Yakin ingin menyetujui kendaraan ini?')"
                                                class="inline-flex items-center px-2 py-1 bg-green-600 text-white rounded hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-700">
                                            <i class="fas fa-check" aria-hidden="true"></i>
                                        </button>
                                    </form>

                                    <form method="POST"
                                          action="{{ route('carbon-credits.reject', $vehicle->id) }}"
                                          style="display: inline;">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                                title="Tolak"
                                                onclick="return confirm('Yakin ingin menolak kendaraan ini?')"
                                                class="inline-flex items-center px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-700">
